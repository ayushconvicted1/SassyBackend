...
const s3_1 = __importDefault(require("../src/configs/s3"));
const db_1 = __importDefault(require("../src/configs/db"));
const BUCKET_NAME = process.env.AWS_S3_BUCKET_NAME;
const DRY_RUN = !process.argv.includes("--delete");
async function getAllDatabaseImageUrls() {
    console.log("📊 Fetching all image URLs from database...");
    // Get all media URLs
    const mediaImages = await db_1.default.media.findMany({
        select: { url: true },
    });
    // Get all home page image URLs
    const homePageImages = await db_1.default.homePageImage.findMany({
    ...
    homePageImages.forEach((img) => {
        if (img.imageUrl)
            dbUrls.add(img.imageUrl);
        if (img.mobileImageUrl)
            dbUrls.add(img.mobileImageUrl);
    });
