...
exports.getTagsAndCategories = exports.applyOfferCode = exports.validateOfferCode = exports.deleteOffer = exports.updateOffer = exports.getOfferById = exports.getAllOffers = exports.createOffer = void 0;
const db_1 = __importDefault(require("../configs/db"));
// Create a new offer
const createOffer = async (req, res) => {
    try {
        const { code, name, description, discountType, discountValue, minOrderValue, maxDiscount, applicableTags, applicableCategories, startDate, endDate, usageLimit, } = req.body;
        // Validation
        ...
        const existingOffer = await db_1.default.offer.findUnique({
...
const getAllOffers = async (req, res) => {
    try {
        ...
        const [offers, total] = await Promise.all([
            db_1.default.offer.findMany({
                where,
                skip,
                take: Number(limit),
                orderBy: { createdAt: "desc" },
            }),
            db_1.default.offer.count({ where }),
        ]);
        ...
        const enhancedOffers = offers.map(offer => {
            ...
            const tagContexts = [];
            offer.applicableTags.forEach((tagName) => {
...
const getOfferById = async (req, res) => {
    try {
        const { id } = req.params;
        const offer = await db_1.default.offer.findUnique({
...
const updateOffer = async (req, res) => {
    try {
        ...
        if (updateData.discountType === "FIXED_AMOUNT" &&
            updateData.discountValue &&
        ...
        const offer = await db_1.default.offer.update({
            where: { id: parseInt(id) },
            data: updateData,
        });
        res.json(offer);
...
const deleteOffer = async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.default.offer.delete({
            where: { id: parseInt(id) },
        });
...
const validateOfferCode = async (req, res) => {
    try {
        ...
        const applicableItems = items.filter((item) => {
            ...
            const isEligible = offer.applicableTags.some((tag) => productTags.includes(tag)) ||
                offer.applicableCategories.some((category) => productCategory === category);
            console.log(`Product ${product.name}: Tags=[${productTags.join(", ")}], Category=${productCategory}, Eligible=${isEligible}`);
            return isEligible;
        });
        ...
        switch (offer.discountType) {
            ...
            case "BOGO":
                ...
                if (sortedItems.length > 0) {
                    ...
                    const product = products.find((p) => p.id === mostExpensiveItem.productId);
...
exports.validateOfferCode = validateOfferCode;
...
exports.applyOfferCode = applyOfferCode;
...
exports.getTagsAndCategories = getTagsAndCategories;
