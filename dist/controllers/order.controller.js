...
exports.updateOrderStatus = exports.verifyPayment = exports.checkout = exports.getOrders = void 0;
const db_1 = __importDefault(require("../configs/db"));
const razorpay_1 = __importDefault(require("../configs/razorpay"));
const crypto_1 = __importDefault(require("crypto"));
const whatsapp_service_1 = require("../services/whatsapp.service");
const getOrders = async (req, res) => {
    try {
        const userId = req?.user?.userId;
        console.log("getOrders - userId from token:", userId, "type:", typeof userId);
        console.log("getOrders - req.user:", req.user);
        if (!userId)
            return res.status(401).json({ error: "Unauthorized" });
        // Ensure userId is a number
        const numericUserId = typeof userId === "string" ? parseInt(userId, 10) : userId;
        console.log("getOrders - numericUserId:", numericUserId, "type:", typeof numericUserId);
        console.log("getOrders - Querying with userId:", userId, "type:", typeof userId);
        ...
        console.log("getOrders - All orders for user", numericUserId, ":", userOrders);
        // Debug: Check only paid orders for user
        const paidOrders = await db_1.default.order.findMany({
            where: { userId: numericUserId, status: "paid" },
            select: { id: true, userId: true, status: true },
        });
        console.log("getOrders - Paid orders for user", numericUserId, ":", paidOrders);
...
const checkout = async (req, res) => {
    try {
        ...
        const { userDetails, items, offer, totals, paymentMethod = "razorpay", } = req.body;
        ...
        if (paymentMethod === "cod") {
            ...
            try {
                await (0, whatsapp_service_1.sendOrderConfirmation)(address.phoneNumber, order.id, finalTotal, "pending");
            }
            catch (whatsappError) {
...
const verifyPayment = async (req, res) => {
    try {
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, orderId, // DB orderId
         } = req.body;
        ...
        if (razorpay_signature === expectedSign) {
            ...
            const order = await db_1.default.$transaction(async (prisma) => {
        ...
        try {
            ...
            if (originalOrder.phoneNumber) {
                if (status === "shipped" && waybillNumber) {
                    await (0, whatsapp_service_1.sendShippingUpdate)(originalOrder.phoneNumber, numericOrderId, waybillNumber);
                }
                else {
                    await (0, whatsapp_service_1.sendOrderStatusUpdate)(originalOrder.phoneNumber, numericOrderId, status);
                }
            }
        }
        catch (whatsappError) {
