...
class AnalyticsService {
    ...
    async getTotalOrders(startDate, endDate) {
        return await db_1.default.order.count({
            where: {
                createdAt: {
                    gte: startDate,
                    lte: endDate,
    ...
    async getTotalUsers(startDate, endDate) {
        return await db_1.default.user.count({
            where: {
                createdAt: {
                    gte: startDate,
                    lte: endDate,
    ...
    async getTotalProducts() {
        return await db_1.default.product.count({
            where: {
                isAvailable: true,
            },
        });
    ...
    async getTopSellingProducts(startDate, endDate, limit) {
        const result = await db_1.default.orderItem.groupBy({
            by: ['productId'],
            where: {
                order: {
                    createdAt: {
    ...
    async getLowPerformingProducts(startDate, endDate, limit) {
    ...
    async getCategoryPerformance(startDate, endDate) {
        const result = await db_1.default.category.findMany({
            include: {
                products: {
                    include: {
                        orderItems: {
        ...
        return result.map(category => {
            const totalSales = category.products.reduce((sum, product) => sum + product.orderItems.reduce((itemSum, item) => itemSum + item.quantity, 0), 0);
            const revenue = category.products.reduce((sum, product) => sum + product.orderItems.reduce((itemSum, item) => itemSum + (item.quantity * Number(item.price)), 0), 0);
            return {
    ...
    async getRevenueOverTime(startDate, endDate, groupBy) {
