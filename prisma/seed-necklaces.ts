...
async function main() {
  ...
  const media = await Promise.all([
    ...
    prisma.media.create({
      data: {
        url: "https://images.unsplash.com/photo-1596944924616-7b384c8c2e0a?w=800&h=800&fit=crop",
        mimeType: "image/jpeg",
        type: "product",
      },
    }),
    ...
    prisma.media.create({
      data: {
        url: "https://images.unsplash.com/photo-1608043152251-1df7f8e4c1c7?w=800&h=800&fit=crop",
        mimeType: "image/jpeg",
        type: "product",
      },
    }),
    prisma.media.create({
      data: {
        url: "https://images.unsplash.com/photo-1617038220319-276d4f4b0b0e?w=800&h=800&fit=crop",
        mimeType: "image/jpeg",
        type: "product",
      },
    }),
  ...
  // Create 25+ Real Necklace Products
  const necklaces = [
    {
    ...
    {
      name: "Sapphire Teardrop Gold Necklace",
      description:
        "Elegant sapphire teardrop pendant in gold. A sophisticated addition to any collection.",
      price: 1199.99,
      stock: 12,
      tags: ["Gold", "Sapphire"],
      images: [10, 11],
    ...
    {
      name: "Diamond Infinity Gold Necklace",
      description:
        "Symbolic diamond infinity symbol in gold. Perfect for anniversaries and special moments.",
      price: 999.99,
      stock: 16,
      tags: ["Gold", "Diamond"],
      images: [12, 13],
    ...
    {
      name: "Pearl Rope Gold Necklace",
      description:
        "Luxurious pearl rope necklace with gold accents. A statement piece for formal occasions.",
      price: 1299.99,
      stock: 8,
      tags: ["Gold", "Pearl"],
      images: [14, 0],
    ...
    {
      name: "Silver and Gold Mixed Chain",
      description:
        "Unique mixed metal chain necklace. A modern take on traditional jewelry.",
      price: 249.99,
      stock: 38,
      tags: ["Silver", "Gold", "Modern"],
      images: [1, 2],
    },
    {
      name: "Diamond Marquise Gold Necklace",
      description:
        "Elegant marquise cut diamond in gold setting. A sophisticated choice for special events.",
      price: 1799.99,
      stock: 9,
      tags: ["Gold", "Diamond"],
      images: [3, 4],
  ...
  const createdNecklaces = await Promise.all(
    necklaces.map(async (necklace, index) => {
      ...
      console.log(`Creating necklace: ${necklace.name}`);
      console.log(
      ...
      return prisma.product.create({
        data: {
          ...
          sizes: {
            create: [
              { sizeId: sizes[0].id, stock: Math.floor(necklace.stock * 0.2) }, // 14 inches
              { sizeId: sizes[1].id, stock: Math.floor(necklace.stock * 0.3) }, // 16 inches
              { sizeId: sizes[2].id, stock: Math.floor(necklace.stock * 0.3) }, // 18 inches
              { sizeId: sizes[3].id, stock: Math.floor(necklace.stock * 0.15) }, // 20 inches
              { sizeId: sizes[4].id, stock: Math.floor(necklace.stock * 0.05) }, // 22 inches
            ],
          },
        },
      });
